...
import rawDefaultTheme from '../../../../packages/core/src/defaults/centoui.css?raw'
...
function preview() {
  ...
  const { schema, values, hasProps } = storeToRefs(store)
...
function theme() {
  ...
  const hasCustomTheme = computed(() =>
    customTheme.value.trim() !== rawDefaultTheme.trim(),
  )
